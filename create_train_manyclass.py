...
# vaih dataset paths
vai_images_path = "./Vaihingen/top"
vai_labels_path = "./Vaihingen/labels"
vai_image_size = 500

# the splitted pots training dataset paths
pots_images_path_train = "./Potsdam/train/images"
pots_labels_path_train = "./Potsdam/train/labels"
pots_image_size = 1000

# map(RGB -> label)
class0 = np.array([255, 255, 255])  #Impervious surface
class1 = np.array([0, 0, 255])   #Building
class2 = np.array([0, 255, 255])    #Low vegetation
class3 = np.array([0, 255, 0])       #Tree
class4 = np.array([255, 255, 0])     #Car 
class5 = np.array([255, 0, 0])   #Clutter/background 
...
def mkdir(path):
    """create a single empty directory if it didn't exicdt
...
if __name__ == "__main__":
    ...
    # mkdirs(['./vaih_pots/trainA/images', './vaih_pots/trainA/labels',
    ...
    mkdirs(['./vaih_pots/trainB/images', './vaih_pots/trainB/labels'])
